...
def glob(root):
    ...
    for f in os.listdir(root):
        if f == 'splits':
            continue
        _dir = os.path.join(root, f)
        if os.path.isdir(_dir):
            files.extend(glob(_dir))
        else:
            files.append(_dir)
    # files = list(filter(lambda x: (os.path.splitext(x)[1] != '.txt'), files))
    return files

class VIGOR_e(torch.utils.data.Dataset):

    def __init__(self, mode, root = '/home/ma293852/Project/dataset/VIGOR/', args=None):
        super(VIGOR_e, self).__init__()
        ...
        self.filelist = glob(root)
        if self.city != 'all':
            self.filelist = list(filter(lambda x: (self.city in x), self.filelist))

    def __len__(self):
        return len(self.filelist)
    
    def __getitem__(self, idx):
        img_reference = Image.open(self.filelist[idx]).convert('RGB')
        img_reference = self.transform_reference(img_reference)
        filename = self.filelist[idx].replace(self.root, '')
        filename = os.path.splitext(filename)[0]

        return img_reference, filename
...
if __name__ == "__main__":
    ...
    args = Args()
    dataset = VIGOR_e(mode='ref', args=args)
