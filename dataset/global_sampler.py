...
def matmul(a, b):
    ...
    m = np.memmap(f'{filename}.memmap', mode='w+', shape=(a.shape[0], b.shape[1]), dtype=np.float32)
...
class DistributedMiningSampler(Sampler[T_co]):
    ...
    def generate_indices_sim(self):
        ...
        if self.epoch >= self.mining_start:
            ...
            product_train, filename = matmul(self.queue[:,:,:,1].mean(axis=1), np.transpose(self.queue[mining_pool,:,:,0].mean(axis=1)))
