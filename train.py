...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if not args.cross:
        print(f"Info: training on {args.city} city dataset split")
...
def validate(val_query_loader, val_reference_loader, model, args):
    ...
    with torch.no_grad():
        ...
        for i, (images, indexes, atten) in enumerate(val_reference_loader):
            ...
            #     ix = indexes[k].item()
            #     file_id = val_reference_loader.dataset.id_test_list[ix][2] 
            #     np.save(f"./DATASET_CVUSA_Features/bingmap/{file_id}.npy", reference_embed[k].detach().cpu().numpy())
        ...
        for i, (images, indexes, labels) in enumerate(val_query_loader):
            ...
            #     ix = indexes[k].item()
            #     file_id = val_query_loader.dataset.id_test_list[ix][2]
            #     np.save(f"./DATASET_CVUSA_Features/streetview/{file_id}.npy", query_embed[k].detach().cpu().numpy())
